...
def encontrar_proxima_combinacao(palavras, ultima_base, ultima_penultima, ultima_completa):
    """Encontra a próxima combinação a ser testada"""
    try:
        base_idx = palavras.index(ultima_base)
        penultima_idx = palavras.index(ultima_penultima)
        start_k_idx = palavras.index(ultima_completa)
        # Avançar para a próxima palavra completa
        if start_k_idx + 1 < len(palavras):
            return base_idx, penultima_idx, start_k_idx + 1
        # Se chegou ao final das palavras completas, avançar para próxima palavra penultima
        elif penultima_idx + 1 < len(palavras):
            return base_idx, penultima_idx + 1, 0
        # Se chegou ao final das palavras penultima, avançar para próxima palavra base
...
def salvar_ultima_combinacao(arquivo="ultimo.txt", palavra_base="", palavra_penultima="", palavra_completa=""):
    """Salva a combinação atual no arquivo"""
    palavras = [palavra_base] * 10 + [palavra_penultima, palavra_completa]
...
def criar_mnemonic_repetido(palavra_base, palavra_penultima, palavra_completa):
    """Cria mnemonic com palavra_base repetida 10 vezes + palavra_penultima + palavra_completa"""
    palavras = [palavra_base] * 10 + [palavra_penultima, palavra_completa]
...
def main():
    ...
    ultima_base, ultima_penultima, ultima_completa, ultimo_mnemonic = carregar_ultima_combinacao("ultimo.txt")
    ...
    if ultima_base and ultima_penultima and ultima_completa:
        print(f"Última combinação testada: {ultimo_mnemonic}")
        base_idx, start_j_idx, start_k_idx = encontrar_proxima_combinacao(palavras, ultima_base, ultima_penultima, ultima_completa)
        if base_idx is None:
            print("Todas as combinações já foram testadas!")
            return
    else:
        print("Nenhum checkpoint encontrado, começando do início...")
        base_idx, start_j_idx, start_k_idx = 0, 0, 0
    
    print(f"Continuando da posição: palavra base #{base_idx+1} ('{palavras[base_idx]}'), palavra penultima #{start_j_idx+1} ('{palavras[start_j_idx]}'), palavra completa #{start_k_idx+1} ('{palavras[start_k_idx]}')")
    ...
    try:
        ...
        for i in range(base_idx, len(palavras)):
            ...
            # Determinar índice inicial para palavra penultima e palavra completa
            start_j = start_j_idx if i == base_idx else 0
            
            for j in range(start_j, len(palavras)):
                palavra_penultima = palavras[j]
                start_k = start_k_idx if (i == base_idx and j == start_j) else 0
                
                for k in range(start_k, len(palavras)):
                    palavra_completa = palavras[k]
                    # pular quando as duas últimas palavras forem iguais (evita penultima == completa)
                    if palavra_completa == palavra_penultima:
                        continue
                    contador_total += 1
                    
                    # Criar mnemonic
                    mnemonic = criar_mnemonic_repetido(palavra_base, palavra_penultima, palavra_completa)
                    
                    # Salvar combinação atual no arquivo
                    salvar_ultima_combinacao("ultimo.txt", palavra_base, palavra_penultima, palavra_completa)
